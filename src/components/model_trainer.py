...
@dataclass
class ModelTrainerConfig:
    trained_model_file_path = os.path.join("artifacts", "model_fnn.h5")
...
class ModelTrainer:
    ...
    def initiate_model_trainer(self, train_array, test_array):
        try:
            ...
            best_hps = tuner.get_best_hyperparameters(num_trials=1)[0]
            logging.info(f"Best number of layers: {best_hps.get('num_layers')}")
            logging.info(f"Best units per layer: {[best_hps.get(f'units_{i}') for i in range(best_hps.get('num_layers'))]}")
            logging.info(f"Best dropout rates: {[best_hps.get(f'dropout_{i}') for i in range(best_hps.get('num_layers'))]}")
            logging.info(f"Best Learning Rate: {best_hps.get('learning_rate')}")
